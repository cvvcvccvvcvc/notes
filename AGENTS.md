...
автоматически разворачивается на сервер. `main` защищён: не коммитьте и не
отправляйте изменения в него напрямую, выпускайте их pull request из `dev`.

Перед release следуйте [docs/versioning.md](docs/versioning.md): один раз
обновите SemVer на `dev`, выполните полный `npm run verify`, затем создайте и
слейте release pull request из `dev` в `main`. Тег ставится только на уже успешно
развёрнутый commit.
