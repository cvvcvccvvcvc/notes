...
- В полночь незавершённые дела прошедшего дня переходят в «Сегодня»;
  открытый интервал завершается границей исходных суток. Дела сохраняют ID,
  не удаляются и не попадают в историю.
